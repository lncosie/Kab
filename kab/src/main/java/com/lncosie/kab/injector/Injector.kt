...
class Injector {
    companion object {
        fun unbind(self: Any) {
            self.javaClass.annotations.forEach{
                when(it){
                    is ZonedStore-> Zone.getZone(it.zoneName)?.unRegister(self as Store)
                }
            }
        ...
        fun bind(self: Any, context: Any) {
            self.javaClass.annotations.forEach{
                when(it){
                    is ZonedStore-> Zone.getZone(it.zoneName)?.register(self as Store)
                }
            }
